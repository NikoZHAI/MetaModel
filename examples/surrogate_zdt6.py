...
from neuro_surrogate import Population, gaussian_mutator, zdt6,\
                            random_crossover, Individual, nsga_crossover
...
def load_theo():
    return pd.read_csv(filepath_or_buffer='./ZDT/ZDT6.pf', names=['f1', 'f2'],
...
ax.scatter(x, y, c='royalblue', s=1.6, label="Surrogate ZDT-6")
...
plt.grid()
plt.show()
fig.savefig('zdt6_tanh.png', format='png')


for i in range(50):
    ind = Individual(dim=DIMENSION, bounds=[0., 1.])
