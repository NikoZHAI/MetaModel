...
pop = Population(dim=3, size=256, fitness_fun=kursawe,
                 max_generation=25)
...
pop.mutaton_rate = 0.07

# Parametrization
region = 0
params_ea = {'u': 0., 'st': 0.3}
params_surrogate = \
...
m = MLPSurrogate(**params_surrogate)
s = MultiOutputRegressor(m)
...
for i in range(1, 8):
    ...
    while pop.generation <= pop.max_generation:
        ...
        print("Episode: %s, Surrogate generation: %s, True front size: %s" %
              (i, pop.generation, pop.true_front[region].__len__()))
        pop.generation += 1

    # Re-evaluate the surrogate-sampled individuals using the PM
    pop.recalc_fitness_with(fun=kursawe, region=0)
    pop.select(region=region, **params_ea)
    pop.update_front(region=region, **params_ea)
    pop.update_true_front(region=region)
    s.fit(pop.render_features(region=region),
...
def load_theo():
    return pd.read_csv('pareto_Kursawe.txt', names=['f1', 'f2'],
...
plt.grid()
plt.show()
# fig.savefig('surrogated.pdf', format='pdf')


for i in range(50):
    ind = Individual()
    diff = np.subtract(pop.fitness_fun(ind.gene), kursawe(ind.gene))
    print(diff)
